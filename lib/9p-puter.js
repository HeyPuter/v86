...
Virtio9p.prototype.SendError = function(tag, err) {
    ...
    console.log("Got err: " + err.code + "mapped to num: " + POSIX_ERR_CODE_MAP[err.code]);
    ...
    switch(id) {
                        ...
                        puterFS.read(path)
                            ...
                            .then(dat => {
                                puterFS.write(path,new window.parent.Blob([dat.slice(0,size)]), {overwrite: true, dedupeName: false})
                                    .then(() => {
                                        console.log("finished truncating ", path);
                                        resolve();
                                    })
                                    .catch(err => {
        ...
        case 116: // read
            ...
            if(!fidata.rcache) {
                ...
                console.log("Attempting to read: " + fidata.path);
                if(fidata.path.endsWith("idx")) {
                    console.log("idx", new Uint8Array(await (await puterFS.read(path)).arrayBuffer()));
                    // debugger;
                }
                puterFS.read(path)
                    .then(blob => blob.arrayBuffer())
                    .then(function(dat) {
                        ...
                        if(self.shouldAbortRequest(tag)) return;
                        console.error("READ ERROR, QUITE FATAL TOO! " + fidata.path);
                        self.SendError(tag, err);
                        self.SendReply(bufchain);
                    });
            } else {
        ...
        case 118: // write
            ...
            console.log("wrote a wop for ", fidata);
        ...
        case 74: // RENAMEAT
            ...
            console.log("MOVE OP: EXIS:" + oldPath + " POSTOP:" + newPath);
                ...
                puterFS.read(fidata.path)
                    ...
                    .then(arrBuf => {
                        ...
                        puterFS.write(fidata.path, finalized, {overwrite: true, dedupeName: false, createMissingParents: true})
                            .then(() => {
                                console.log("finished writing ", fidata.path);
