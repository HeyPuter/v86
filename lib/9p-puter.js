...
function getQType(type) {
    switch(type) {
        ...
        case true:
            return 0x80;
        case "SYMLINK": // never going to happen, for future ref
            return 0x02;
        default:
            return 0x00;
    }
...
function Virtio9p(filesystem, cpu, bus) {
    ...
    this.puterFS = window.parent.puter.fs;
...
Virtio9p.prototype.shouldAbortRequest = function(tag) {
    ...
    if(shouldAbort) {
        console.debug("Request can be aborted tag=" + tag);
    }
    return shouldAbort;
};
...
Virtio9p.prototype.BuildReply = function(id, tag, payloadsize) {
    ...
    if((payloadsize + 7) >= this.replybuffer.length) {
        console.debug("Error in 9p: payloadsize exceeds maximum length");
    }
...
Virtio9p.prototype.ReceiveRequest = async function(bufchain) {
    ...
    this.addTag(tag);
    // console.debug("size:" + size + " id:" + id + " tag:" + tag);
    // console.log(header);
    switch(id) {
        case 8: // statfs
        ...
        case 12: // tlopen
            ...
            console.debug("[open] fid=" + fid + ", mode=" + mode);
            console.debug("file open " + this.fids[fid].path);
        ...
        case 70: // link
            ...
            self.SendReply(bufchain);
            console.debug("[link] dfid=" + dfid + ", name=" + name);
        ...
        case 18: // mknod
            ...
            var minor = req[4];
            var gid = req[5];
            console.debug("[mknod] fid=" + fid + ", name=" + name + ", major=" + major + ", minor=" + minor + "");
        ...
        case 22: // TREADLINK
            ...
            console.debug("[readlink] fid=" + fid + " name=" + this.fids[fid].dbg_name + " target=" + "idk");
        ...
        case 72: // tmkdir
            ...
            console.debug("[mkdir] fid=" + fid + ", name=" + name + ", mode=" + mode + ", gid=" + gid);
        ...
        case 14: // tlcreate
            ...
            console.debug("[create] fid=" + fid + ", name=" + name + ", flags=" + flags + ", mode=" + mode + ", gid=" + gid);
            puterFS.write(newFilePath).then((stats) => {
                console.log(stats);
        ...
        case 24: // getattr
            ...
            console.debug("[getattr]: fid=" + fid + " name=" + this.fids[fid].dbg_name + " request mask=" + req[1]);
        ...
        case 26: // setattr
                ...
                var path = this.fids[fid].path;
                const promises = [];
                console.log("[setattr]: fid=" + fid + " request mask=" + req[1]);
                console.log("lying about successful setattr, no ext attr support in puter atm");
                if(req[1] & P9_SETATTR_SIZE) {
                    promises.push(
                        new Promise(function(resolve, reject) {
                            ...
                            .then(dat => {
                                puterFS.write(path,new window.parent.Blob([dat.slice(0,3)]))
                                .then(() => {
                                    resolve();
                                })
                                .catch(err => {
        ...
        case 40: // TREADDIR
        // going to be one hell of a case to convert...
        ...
        case 116: // read
            ...
            if(data) {
                console.log("pending tags had some data: ", data);
                _read(data);
                return;
            }
            if(!fidata.rcache)
                ...
                .then(dat => {
                    ...
                    if(self.shouldAbortRequest(tag)) return;
                    console.log("got data for ",path, data);
        ...
        case 118: // write
            ...
            console.debug("[write]: fid=" + fid + " offset=" + offset + " count=" + count + " fidtype=" + this.fids[fid].type);
            if(!fidata.wops) {
                fidata.wops = [];
            }
        ...
        case 74: // RENAMEAT
            ...
            console.debug("[renameat]: oldname=" + oldname + " newname=" + newname);

            puterFS.stat(newPath, (exists) => {

                    puterFS.move(oldPath, newPath).then(function(err) {
                        if(self.shouldAbortRequest(tag)) return;
        ...
        case 76: // TUNLINKAT
            ...
            console.debug("[unlink]: dirfd=" + dirfd + " name=" + name + " flags=" + flags);
        ...
        case 100: // version
            ...
            console.debug("[version]: msize=" + version[0] + " version=" + version[1]);
        ...
        case 104: // attach
            ...
            var fid = req[0];
            var uid = req[4];
            console.debug("[attach]: fid=" + fid + " afid=" + hex8(req[1]) + " uname=" + req[2] + " aname=" + req[3]);
            this.fids[fid] = this.Createfid("/", FID_INODE, uid);
            puterFS.stat("/").then(function(stats) {
                if(self.shouldAbortRequest(tag)) return;
        ...
        case 110: // walk
            ...
            var nwfid = req[1];
            var nwname = req[2];
            console.debug("[walk]: fid=" + req[0] + " nwfid=" + req[1] + " nwname=" + nwname);
            if(nwname === 0) {
            ...
            console.debug("walk in dir " + this.fids[fid].dbg_name + " to: " + walk.toString());
            ...
            if(fidata.wops) {
                ...
                .then(arrBuf => {
                    ...
                    data.set(dat, 0);
                        ...
                        console.log(`to path ${fidata.path}, writing `, finalized);
                        puterFS.write(fidata.path, finalized)
                        .then(() => {
        ...
        case 32: // txattrcreate
            ...
            var attr_size = req[2];
            var flags = req[3];
            console.debug("[txattrcreate]: fid=" + fid + " name=" + name + " attr_size=" + attr_size + " flags=" + flags);
        ...
        case 30: // xattrwalk
            ...
            var newfid = req[1];
            var name = req[2];
            console.debug("[xattrwalk]: fid=" + req[0] + " newfid=" + req[1] + " name=" + req[2]);
        ...
        default:
            console.debug("Error in Virtio9p: Unknown id " + id + " received");
            message.Abort();
